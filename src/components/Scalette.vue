    ...
    <div v-if="artistsLoaded">
      ...
      <b-container class="bv-example-row">
        <b-row class="text-center">
          <b-col cols="3">
            <b-list-group>
              <b-list-group-item v-for="(elem, index) in allEvents" :key="index" @click="findLiveEvents(elem.value)"
                                 button>{{ elem.value }}
              </b-list-group-item>
            </b-list-group>
          </b-col>
...
export default {
  ...
  async mounted() {
    ...
    /*
    * Query per ottenere tutti i gli eventi
    */
    const query = `
  ...
  methods: {
    async findLiveEvents(eventName) {
      ...
      /*
      * Query per ottenere tutti i dettagli sulle performance in scaletta
      */
      query = `
          PREFIX meo: <http://www.modsem.org/musicalEventsOntology#>
          PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
